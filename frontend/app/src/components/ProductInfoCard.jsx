...
const ProductInfoCard = ({ product }) => {
  ...
  const categoryLink = product.category ? `/products/${product.category}` : "/products";
  ...
  return (
    <div className="p-4 border rounded-lg shadow-lg bg-white max-w-md text text-center relative">
      <div className="flex justify-between items-center mb-2">
        ...
        {/* Botón de favoritos */}
        <button
          className={`text-xl ${isFavorite ? "text-blue-500" : "text-gray-400"}`}
      ...
      <div className="flex flex-col justify-center items-start flex-grow w-full">
        <h1 className="text-3xl font-bold mb-2 my-6">
          ...
          {product.name ? product.name.charAt(0).toUpperCase() + product.name.slice(1) : "Producto sin nombre"}
        </h1>
        <p className="text-gray-700 mb-4">{product.description || "Sin descripción disponible"}</p>
        <p className="text-5xl font-light text-gray-500">${product.price ?? "Precio no disponible"}</p>
        ...
        <div className="mt-4 relative">
          ...
          >
            ...
            <ChevronDown className={`w-4 h-4 mr-1 m-1 transition-transform ${dropdownOpen ? "rotate-180" : "rotate-0"}`} />
          </button>
          {dropdownOpen && (
        ...
        <p className="text-sm font-bold text-gray-800 mt-4">Color: {selectedColor || "Seleccione un color"}</p>
        ...
        <button
          className="mt-6  bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 w-full self-center"
        ...
        <button
          className="mt-4 mb-8 bg-blue-100 text-blue-500 px-4 py-2 rounded hover:bg-blue-200 transition w-full self-center"
